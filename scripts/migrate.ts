...
try {
  await client.query(
    "SELECT pg_advisory_lock(hashtext('algaguard-access-service-migrations'))",
  );
  ...
  for (const filename of files) {
    ...
    const applied = await client.query(
      ...
      ["algaguard-access-service", filename],
    );
    if (applied.rows[0]) {
    ...
    try {
      ...
      await client.query(
        ...
        ["algaguard-access-service", filename, sha256],
      );
  ...
  await client.query(
    "SELECT pg_advisory_unlock(hashtext('algaguard-access-service-migrations'))",
  );
  client.release();
  await pool.end();
}
